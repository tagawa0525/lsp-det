...
- 外向きの提出（戦略・順序・規則は `docs/upstream-submissions.md`。文面を作ってユーザーの確認をもらってから出す）: 提出前の準備（済: typescript-language-server のパッチの作り直し（fork の `tsserver-exit-by-signal`、2026-09-09。同日 typescript-language-server/typescript-language-server#1125 として提出）。gopls を health に縮める（2026-09-09、`docs/research/gopls-health-measurement.md`。同日 golang/go#78273 へのコメントと golang/go#81400 として提出）。rust-analyzer の `serverStatus` への field 追加案（2026-09-09、fork の `server-status-readiness`。issue の草案は両案を並べる）。LSP 本体向けの `.proposed.ts`（2026-09-09、fork `tagawa0525/vscode-languageserver-node` の `server-state`。#511 へのコメントと proposal issue の草案も）。残: Serena の再測定）→ 第 1 段（typescript-language-server、Claude Code anthropics/claude-code#76870 への `experimental/serverState` の提案、pyright）→ 第 2 段（Serena の registry、rust-analyzer の両案、gopls の health）→ 第 3 段（12 サーバー、LSP 本体）
