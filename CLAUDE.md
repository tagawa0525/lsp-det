...
- 0.7.0（ADR 0022: 仕様を安定版 1.0 に。ADR 0020 / 0021 の追補: begin の来ない workspace の clangd と nil は `unknown` から始めて保留しない。対外戦略は `docs/upstream-submissions.md`、その批判的レビューは `docs/research/upstream-strategy-review-2026-09.md`）も完了。仕様の版は 1.0 で、以後の変更は版を上げて 11 章に記す。README.md から直接リンクする文書は英語が正（ADR 0017 追補 F）。準備 2（tsls のパッチの作り直し）は済: fork の `tsserver-exit-by-signal`（`onExit` の `if (exitCode)` を外す。受け入れ条件は `typescript_language_server_exits_when_tsserver_is_killed`、再測定は `docs/research/typescript-language-server-readiness-measurement.md` の末尾）。次: 準備 3〜6（gopls を health に縮める、rust-analyzer の field 追加案、`.proposed.ts`、Serena の再測定）→ 第 1 段の提出（文面を作ってユーザーの確認をもらってから出す）
