...
- 0.7.0（ADR 0022: 仕様を安定版 1.0 に。ADR 0020 / 0021 の追補: begin の来ない workspace の clangd と nil は `unknown` から始めて保留しない。対外戦略は `docs/upstream-submissions.md`、その批判的レビューは `docs/research/upstream-strategy-review-2026-09.md`）も完了。仕様の版は 1.0 で、以後の変更は版を上げて 11 章に記す。次: 提出前の準備 2〜7（tsls のパッチの作り直し、gopls を health に縮める、rust-analyzer の field 追加案、`.proposed.ts`、Serena の再測定、README の導入手順）→ 第 1 段の提出（文面を作ってユーザーの確認をもらってから出す）
