...
- 実サーバーの結合テストは `cargo test --test conformance -- --ignored`（65 件。全件は `--test-threads=1` で回す。並列では tsls の 7.3 の Changed が負荷で揺れる。Metals、Expert、Nextflow、HLS、pyrefly、crystalline、Gleam、haxe-language-server、Dart、Sorbet、jdtls、clangd は `nix develop .#servers` で）と `cargo test --test process_lifetime -- --ignored`（4 件）。`TESTED_VERSIONS` を動かすのはこれらを通してから
