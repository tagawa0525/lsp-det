...
- **M4 — gopls の写像（進行中）**: `src/adapter/gopls.rs`（`$/progress` の title からの合成。写像は `adapter::Mapping` trait に統一）。偽上流での準拠テストは完了。残り: 実 gopls への結合テスト（`tests/conformance.rs` の `gopls_*` ignored 4 件）を回し、go.mod 変更時の再発行なし（ソースで確認済み）を実測で確かめ、7.2 / 7.3 の結果で保証を確定する。gopls と go は nixfiles の `feat/go-toolchain` ブランチで追加済み（要リビルド）
