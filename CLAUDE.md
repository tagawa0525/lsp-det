...
- **v0.2（ADR 0010、2026-09-03 決定）**: M5 pyright の写像（basedpyright と共有。信号は `window/logMessage` のファイル列挙完了で `$/progress` ではない。pyright は `serverInfo` を返さないので起動ログの名乗りで選ぶ。ADR 0011、`docs/research/pyright-readiness-measurement.md`）→ M6 typescript-language-server の写像（クラッシュ時も progress end が来るので `window/logMessage` で health を合成。猶予時間は置かない）→ M7 Serena 統合（`ls_base_cmd` で python / typescript を lsp-det 経由に）。各段は M4 と同じ手順（flake に追加 → 実測記録 → RED → GREEN → 実サーバー結合テスト → `TESTED_VERSIONS`）。README と仕様の英訳、rust-analyzer / gopls への上流 issue は v0.2 と独立に進めてよい
