...
- **M2（進行中）**: 準拠テストスイート（中心成果物）+ 拡張 S surface + ゲート（rust-analyzer）。4 PR に分割。
  ...
  - PR 2.5（ADR 0008 の実装）: `Readiness::Unknown` / `Health::Unknown`、状態の保持を `src/tracker.rs` に分離（アダプタは `src/adapter.rs` で解釈だけを担う）。アダプタなしは両軸 `unknown` から始まり消失で `dead`。準拠テストにアダプタなしの被験者を追加
