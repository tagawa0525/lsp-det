...
成功基準は「仕様・上流側と下流側それぞれの準拠テスト・両半身の参照実装が自己無矛盾で、rust-analyzer と gopls に当てて通ること」（ADR 0009）。作者の Claude Code 環境での稼働は成功基準ではなく観測手段。
...
- **M3 — 下流側**: 保留キュー・キャンセル・`shutdown` 時のエラー応答・`error` での即時エラー・再インデックス待機。判定表は v0.1-design 4.3 が正。下流側の準拠テスト（仕様 9.1）を先に書く（RED）。時間の非常口は作らない
