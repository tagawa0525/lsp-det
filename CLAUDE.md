...
- 英語が正: `README.md`、`docs/spec/server-state.md`、そして `README.md` から直接リンクする 6 本（`docs/vision.md`、`docs/v0.1-design.md`、`docs/research/claude-code-dogfooding.md`、`docs/adr/README.md`、`scripts/upstream/README.md`、`dogfood/serena/README.md`。追補 F）、および提出物がリンクで指す調査報告 2 本（`docs/research/readiness-vocabulary-corpus.md`、`docs/research/workspace-symbol-truncation-measurement.md`。追補 G）。日本語版（同名の `.ja.md`）は**同じコミット**で追従させ、見出しの構成を 1 対 1 に保つ。レビューは日本語版で行う
...
- 0.7.0（ADR 0022: 仕様を安定版 1.0 に。ADR 0020 / 0021 の追補: begin の来ない workspace の clangd と nil は `unknown` から始めて保留しない。対外戦略は `docs/upstream-submissions.md`（上流ごとの文面と反応は `docs/upstream-submissions/<上流>.md`）、その批判的レビューは `docs/research/upstream-strategy-review-2026-09.md`）も完了。仕様の版は 1.0（v0.7.0 で凍結）から始まり、以後の変更は版を上げて 11 章に記す（現在は 1.1。ADR 0023）。README.md から直接リンクする文書は英語が正（ADR 0017 追補 F）。準備 2（tsls のパッチの作り直し）は済で、typescript-language-server/typescript-language-server#1125 として提出し（2026-09-09。第 1 段の最初の提出）、2026-09-23 にマージ、v6.0.1 で配布（flake の版が 6.0.1 以上になったら tsls のクラッシュの準拠テストを上流消失の形に書き換える）: fork の `tsserver-exit-by-signal`（`onExit` の `if (exitCode)` を外す。受け入れ条件は `typescript_language_server_exits_when_tsserver_is_killed`、再測定は `docs/research/typescript-language-server-readiness-measurement.md` の末尾）。準備 3（gopls を health に縮める）も済: `docs/research/gopls-health-measurement.md`（失敗中の要求は明示的なエラー、失敗の信号は `window` を宣言しないクライアントには type 4 の showMessage に落ちる、回復後の go.mod 変更に約 1 秒の窓）。golang/go#78273 へのコメントと窓の新規 issue golang/go#81400 として提出済み（2026-09-09）。準備 4（rust-analyzer の `serverStatus` への field 追加案）も済: fork の `server-status-readiness`（3 値の `readiness`。issue が指すので残す）。2026-09-13 にメンテナ（ChayimFriedman2）が `ready: bool` の単一 field に簡略化した案 A を選んだので、fork の `server-status-ready`（c6b47b9bef。`ServerStatusParams` に `ready` を足し、読み込み中は false、読み込み後は priming 中も true。初期状態は `ready: false`。遷移を観る slow-tests 2 件）に作り直し、fork の CI（CI 専用ブランチで `github.repository` の条件を外す）で全ジョブを通してから rust-lang/rust-analyzer#23362 として提出済み（2026-09-15）。受け入れ条件は `rust_analyzer_reports_ready_in_server_status`、写像は field があれば true → ready、false → indexing。実測（`scripts/rust-analyzer/status-probe.py`、`docs/research/rust-analyzer-quiescent-measurement.md` の末尾）で Cargo.toml のないディレクトリでは最初のロードの前に `quiescent: true` が送られることを確認。issue の草案は `docs/upstream-submissions/rust-analyzer.md`。準備 5（LSP 本体向けの `proposed.serverState.ts`）も済: fork `tagawa0525/vscode-languageserver-node` の `server-state`（`proposed.serverState.ts` と同名の `.md`、`api.ts` の `Proposed`、`metaModel.json`。方法名は採用後の `workspace/serverState` / `workspace/serverStateChanged`、capability は `workspace.serverState: boolean` と `serverStateProvider`）。microsoft/language-server-protocol#511 へのコメントと proposal issue の草案は `docs/upstream-submissions/lsp.md`。準備 6（Serena の再測定）も済: 上流 HEAD `701e7c84` で前回と同じ結果（`docs/research/serena-integration-measurement.md` の末尾）。クラッシュ検知が 2 回目以降の横断要求に効かない穴は HEAD にも oraios/serena#1978 にも残り、修正は fork `tagawa0525/serena` の `tsserver-crash-on-request-path`（latch の前で `_raise_if_crashed()`。受け入れ条件は `scripts/serena/probe.py` の `CRASH=1 VIA_LSP_DET=0` がコード 0 で終わること）。不具合 4 件は HEAD に残る。PR・issue 4 件・registry（oraios/serena#1988）への提案の草案は `docs/upstream-submissions/serena.md`。準備 7（README の Nix を使わない導入手順の点検）も済: v0.7.0 の Linux のバイナリが glibc 2.39 に動的リンクされ「静的」の記述が誤りだったので、Release の Linux を musl の静的リンクに切り替え（ADR 0012 追補 2026-09-09。次の `v*` のタグから）、README を事実に合わせた。0.7.1（2026-09-09）は Linux の Release バイナリを musl に直した版で、lsp-det の挙動は 0.7.0 と同じ。第 1 段の提出は済（2026-09-09。Claude Code へのコメント 4 件 anthropics/claude-code#76870、anthropics/claude-code#82416、anthropics/claude-code#85225、anthropics/claude-code#16360 と新規 issue 2 件 anthropics/claude-code#93103（`shutdown` の `params: {}`）、anthropics/claude-code#93104（`didClose`）、pyright の enhancement request microsoft/pyright#11723。出した文面は `docs/upstream-submissions/<上流>.md` にそのまま残す。出す直前の再測定は `scripts/claude-code/tee-probe` で CC 2.1.266）。第 2 段も提出済み（2026-09-10。Serena: PR oraios/serena#2007 と issue oraios/serena#2003〜#2006、oraios/serena#1988 へのコメント。rust-analyzer: rust-lang/rust-analyzer#23331 で両案を並べ、fork の両ブランチは upstream master に rebase 済み。2026-09-13 にメンテナが `ready: bool` に簡略化した案 A を選んだ。準備 4 の項）。golang/go#81400 には Go チームの修正 CL 830924 が出て、4 ビルド（CL、親、両方 + デバウンス CL 830844）で検証しコメント済み。CL 830844 は `didChangeWatchedFiles` の直後の要求に変更前の答えを返し 7.3.2 が落ちるので golang/go#81408 にコメント済み（2026-09-12。`docs/research/gopls-health-measurement.md` の末尾、probe は `scripts/gopls/health-probe.py`）。CL が入った版が出たら 7.3.2 を当て直す。rust-lang/rust-analyzer#23362 にはメンテナが 2026-09-15 に AI 方針（`AI_POLICY.md`: 返信・PR 本文は人が自分の言葉で書く、AI 利用は開示、自律エージェントの PR は閉じる）を読むよう指摘。2026-09-23 にユーザーが日本語で返信し、Claude の英訳を引用ブロックで添えた。以後 rust-analyzer への文面はユーザーが書く。以後の提出は相手の AI 方針を先に確かめる（`docs/upstream-submissions.md` の規則）。ADR 0023（2026-09-25 採用）: solution のない multi-project の workspace で tsls の `coverage` の宣言が破れていた（`docs/research/typescript-language-server-no-solution-coverage.md`）ので、写像は workspace の各フォルダの根の設定ファイル 1 つが全ソースを含むときだけ `coverage` を宣言する（規則 R1。`src/adapter/typescript_layout.rs`）。宣言の後にクライアントが設定ファイルの変化を知らせたら配置を読み直し、完全でなくなったら（または範囲の外の新規ソース、フォルダの追加で）`readiness` を `unknown` に（追補 2026-09-26）。仕様は 1.1（8.2 の 5 に workspace の形）。規則 R2（根の solution）は solution の fixture の準拠テストと一緒に後で。次: 応答待ち（催促は 2 週間後に 1 回だけ）。第 3 段（LSP 本体）は第 2 段のどれかに反応があってから
