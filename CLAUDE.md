...
- 上流に出す変更は `scripts/upstream/README.md` の手順でローカルに確かめる（4 上流のパッチは fork のブランチに用意済み。上流への PR はユーザー確認のうえで出す）（`reference/` の clone をビルドして `target/upstream/bin` を PATH の先頭に置き、`tests/upstream_dev.rs` の受け入れ条件と準拠テストを当てる）。Serena 側は `scripts/serena/probe.py`
