...
| nil                          | 固定の文字列 token 3 つの `$/progress`（flake の情報を読む段階）。flake.lock 自身の読み込みには信号がない                                                                                                         | `window/showMessage` の type 1 / 2 → `error` / `warning`。いずれかの token の begin → `ok`                                       | 2026-07-23（文書に閉じる coverage のみ。`references` は要求のあった文書に閉じ、freshness は宣言しない） |
