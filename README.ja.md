...
| clangd                       | `$/progress` "indexing"（token `backgroundIndexProgress`。最初の `didOpen` で compilation database が見つかったときだけ出る）                                                                                     | なし（`unknown`）                                                                                                                | 21.1.8（coverage のみ。データベースがなければトークンが来ず横断リクエストは保留されたまま）             |
