...
| nixd                         | `$/progress` "evaluating nixpkgs entries" / "evaluating nixos options"（token は JSON の数値。`initialize` の直後から 2 本が並行する）                                                                            | なし（`unknown`）                                                                                                                | なし。`references` が要求のあった文書に閉じ、この仕様にまだ名前のない範囲になる（2.9.2 で実測）      |
| nil                          | 固定の文字列 token 3 つの `$/progress`（flake の情報を読む段階）。flake.lock 自身の読み込みには信号がない                                                                                                         | `window/showMessage` の type 1 / 2 → `error` / `warning`。いずれかの token の begin → `ok`                                       | なし。`references` が要求のあった文書に閉じ、この仕様にまだ名前のない範囲になる（2026-07-23 で実測） |
