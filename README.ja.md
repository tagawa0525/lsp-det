...
既存の語彙との対応（rust-analyzer の `experimental/serverStatus`、gopls の `$/progress`、pyright の起動ログ、typescript-language-server の progress とクラッシュログ、jdtls の `language/status`、clangd の背景索引の `$/progress`）は仕様 10 章。
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| rust-analyzer                | `experimental/serverStatus` の `quiescent`                                                                                                                                                                                             | 同 `health`                                                                                                                      | 1.97.1、1.98.0、2026-08-03                                                                                           |
...
| typescript-language-server   | `$/progress` "Initializing JS/TS language features…"                                                                                                                                                                                   | "[tsserver] Exited" のログ → `error`                                                                                             | TypeScript 5.9.3（`coverage` は workspace のフォルダが 1 つで、その根の設定 1 つが全ソースを含むときだけ。ADR 0023） |
...
| haxe-language-server（Haxe） | `$/progress` "Haxe: Building Cache..." / "Haxe: Parsing Classpaths..." / "Haxe: Building Refactoring Cache…..."（`window/logMessage` の "Haxe Path: " で識別。`workspace/didChangeConfiguration` の後にだけ来る）                      | `window/showMessage` の version-check / invalid-argument エラーと `haxe/haxeKeepsCrashing` で `error`、"Haxe connected!" で `ok` | なし。版がプロトコルに現れない                                                                                       |
| Dart analysis server         | `$/progress` "Analyzing…"（token `ANALYZING`）                                                                                                                                                                                         | なし（`unknown`）                                                                                                                | 3.13.0                                                                                                               |
| jdtls                        | `language/status` の `ServiceReady`（`$/progress` はあるが読まない: JDT 自身の検索が索引の完了を待つ）                                                                                                                                 | `language/status` の `ProjectStatus` "OK" / "WARNING" と `Error`。`.java` でない（プロジェクト）URI への severity 1 の診断       | 1.60.0-SNAPSHOT                                                                                                      |
| clangd                       | `$/progress` "indexing"（token `backgroundIndexProgress`。最初の `didOpen` で compilation database が見つかったときだけ出る）。その `didOpen` で観測者自身が探してもデータベースが見つからなければ `readiness` は `unknown` から始まる | なし（`unknown`）                                                                                                                | 21.1.8（coverage のみ）                                                                                              |
| nixd                         | `$/progress` "evaluating nixpkgs entries" / "evaluating nixos options"（token は JSON の数値。`initialize` の直後から 2 本が並行する）                                                                                                 | なし（`unknown`）                                                                                                                | 2.9.2（文書に閉じる coverage のみ。`references` は要求のあった文書に閉じ、freshness は宣言しない）                   |
| nil                          | 固定の文字列 token 3 つの `$/progress`（flake の情報を読む段階）。flake.lock 自身の読み込みには信号がない。workspace folder のどの `flake.lock` の root にも `nixpkgs` の入力がなければ `unknown` から始める                           | `window/showMessage` の type 1 / 2 → `error` / `warning`。いずれかの token の begin → `ok`                                       | 2026-07-23（文書に閉じる coverage のみ。`references` は要求のあった文書に閉じ、freshness は宣言しない）              |
| Sorbet                       | `sorbet/showOperation`（通知のメソッドそのもので識別。lsp-det は自分が起動したコマンドが `sorbet` か `srb` のときだけ opt-in を注入する）                                                                                              | なし（`unknown`）                                                                                                                | なし。版がプロトコルのどこにも現れない                                                                               |
...
- **Release のバイナリ**: `v*` の tag ごとに [GitHub Releases](https://github.com/tagawa0525/lsp-det/releases) にプラットフォームごとに 1 つのバイナリが付く: `lsp-det-x86_64-unknown-linux-musl` と `lsp-det-aarch64-unknown-linux-musl`（静的リンクなのでどのディストリビューションでも動く。v0.7.1 以降。v0.7.0 までは glibc 2.39 が要る `*-unknown-linux-gnu` のバイナリ）、`lsp-det-x86_64-apple-darwin` と `lsp-det-aarch64-apple-darwin`、`lsp-det-x86_64-pc-windows-msvc.exe`（OS だけに依存する）。自分のプラットフォームのものを落とし、`lsp-det`（Windows は `lsp-det.exe`）に改名して実行権限を付け、`PATH` に置く。バイナリは署名していない: macOS ではブラウザで落としたものが隔離され、`xattr -d com.apple.quarantine lsp-det` するまで Gatekeeper が実行を拒む（`curl` で落としたものは隔離されない）。引数なしで `lsp-det` を実行すると使い方を出して 2 で終わるので、バイナリが動くことの確認はそれで足りる
