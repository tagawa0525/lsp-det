...
| nil                         | `$/progress` on 3 fixed string tokens for the phases that read flake information; flake.lock's own read has no signal                                                                                                       | `window/showMessage` type 1 / 2 → `error` / `warning`; the begin of any token → `ok`                                       | 2026-07-23 (document-scoped coverage only; `references` is limited to the requesting document, no `freshness`)   |
