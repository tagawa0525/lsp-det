...
| clangd                      | `$/progress` "indexing" (token `backgroundIndexProgress`, sent once a compilation database is found at the first `didOpen`)                                                                                                 | None (`unknown`)                                                                                                           | 21.1.8 (coverage only; without a compilation database the token never arrives, so cross-file requests stay held) |
