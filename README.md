...
| nixd                        | `$/progress` "evaluating nixpkgs entries" / "evaluating nixos options" (JSON-number tokens, 2 running in parallel right after `initialize`)                                                                                 | None (`unknown`)                                                                                                           | None: `references` is limited to the requesting document, a scope not yet named in this protocol (verified with 2.9.2)      |
| nil                         | `$/progress` on 3 fixed string tokens for the phases that read flake information; flake.lock's own read has no signal                                                                                                       | `window/showMessage` type 1 / 2 → `error` / `warning`; the begin of any token → `ok`                                       | None: `references` is limited to the requesting document, a scope not yet named in this protocol (verified with 2026-07-23) |
