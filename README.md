...
Chapter 10 of the spec maps existing vocabularies onto the protocol: rust-analyzer's `experimental/serverStatus`, gopls's `$/progress`, pyright's startup log, typescript-language-server's progress and crash log, jdtls's `language/status`, and clangd's background-index `$/progress`.
...
**The upstream side** stands in for the language server. It selects a mapping by the name the server gives in `InitializeResult.serverInfo` (or, failing that, in its startup log or in what its `InitializeResult` declares), maps the server's vocabulary onto `ServerState`, and adds `serverStateProvider` to the `InitializeResult`. Guarantees are declared only for versions that passed conformance tests 7.2 and 7.3. If the server speaks the protocol itself, nothing is added and everything passes through.

| Language server             | Readiness signal                                                                                                                                                                                                                     | Health signal                                                                                                              | Versions with guarantees                                                                                               |
| --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| rust-analyzer               | `quiescent` in `experimental/serverStatus`                                                                                                                                                                                           | `health` in the same notification                                                                                          | 1.97.1, 1.98.0, 2026-08-03                                                                                             |
...
| typescript-language-server  | `$/progress` "Initializing JS/TS language features…"                                                                                                                                                                                 | The "[tsserver] Exited" log → `error`                                                                                      | TypeScript 5.9.3 (`coverage` only for a single workspace folder whose one root config takes in every source, ADR 0023) |
...
| Nextflow language server    | `publishDiagnostics` for every `*.nf` under the workspace folders after the `$/progress` "Initializing" end (the observer walks the folders itself); `didOpen` / `didChange` / `didClose` until that document's diagnostics          | None (`unknown`)                                                                                                           | None: the version is not observable (no `serverInfo`)                                                                  |
| haskell-language-server     | None readable: `$/progress` is created only for sessions longer than 1 second and covers slices; `references` grows while indexing. `unknown`                                                                                        | A `source: "cradle"` error diagnostic → `error`                                                                            | None: readiness is not observed                                                                                        |
| pyrefly                     | None: the initial index reports only to stderr; "Pyrefly: Rechecking" covers open files only. `unknown`                                                                                                                              | None (`unknown`)                                                                                                           | None: no mapping                                                                                                       |
| crystalline                 | `window/logMessage` "LSP server is ready." (selected by "[workspace] Found projects:"); `$/progress` is a per-request compilation and is not read                                                                                    | None (`unknown`)                                                                                                           | None: the version is not observable (no `serverInfo`)                                                                  |
| Gleam                       | `$/progress` "Downloading Gleam dependencies" (sent right after `initialized` even with nothing to download)                                                                                                                         | None (`unknown`)                                                                                                           | None: the version never appears in the protocol                                                                        |
| haxe-language-server (Haxe) | `$/progress` "Haxe: Building Cache..." / "Haxe: Parsing Classpaths..." / "Haxe: Building Refactoring Cache…..." (identified by "Haxe Path: " in `window/logMessage`, sent only after `workspace/didChangeConfiguration`)             | `window/showMessage` version-check / invalid-argument errors, `haxe/haxeKeepsCrashing` → `error`; "Haxe connected!" → `ok` | None: the version never appears on the protocol                                                                        |
| Dart analysis server        | `$/progress` "Analyzing…" (token `ANALYZING`)                                                                                                                                                                                        | None (`unknown`)                                                                                                           | 3.13.0                                                                                                                 |
| jdtls                       | `ServiceReady` in `language/status` (`$/progress` exists but is not read: JDT's own search waits for the index)                                                                                                                      | `ProjectStatus` "OK" / "WARNING" and `Error` in `language/status`; a severity-1 diagnostic on a non-`.java` (project) URI  | 1.60.0-SNAPSHOT                                                                                                        |
| clangd                      | `$/progress` "indexing" (token `backgroundIndexProgress`, sent once a compilation database is found at the first `didOpen`); if the observer's own search for one at that `didOpen` finds none, `readiness` starts `unknown` instead | None (`unknown`)                                                                                                           | 21.1.8 (coverage only)                                                                                                 |
| nixd                        | `$/progress` "evaluating nixpkgs entries" / "evaluating nixos options" (JSON-number tokens, 2 running in parallel right after `initialize`)                                                                                          | None (`unknown`)                                                                                                           | 2.9.2 (document-scoped coverage only; `references` is limited to the requesting document, no `freshness`)              |
| nil                         | `$/progress` on 3 fixed string tokens for the phases that read flake information; flake.lock's own read has no signal; starts `unknown` instead if no workspace folder's `flake.lock` root has a `nixpkgs` input                     | `window/showMessage` type 1 / 2 → `error` / `warning`; the begin of any token → `ok`                                       | 2026-07-23 (document-scoped coverage only; `references` is limited to the requesting document, no `freshness`)         |
| Sorbet                      | `sorbet/showOperation` (identified by the notification method itself; lsp-det injects the opt-in only for a command it launched named `sorbet` or `srb`)                                                                             | None (`unknown`)                                                                                                           | None: the version never appears in the protocol                                                                        |
...
- **Release binaries**: each `v*` tag attaches one binary per platform to [GitHub Releases](https://github.com/tagawa0525/lsp-det/releases): `lsp-det-x86_64-unknown-linux-musl` and `lsp-det-aarch64-unknown-linux-musl` (statically linked, so they run on any distribution; from v0.7.1 on, since v0.7.0 and earlier shipped `*-unknown-linux-gnu` binaries that need glibc 2.39), `lsp-det-x86_64-apple-darwin` and `lsp-det-aarch64-apple-darwin`, and `lsp-det-x86_64-pc-windows-msvc.exe` (these depend only on the OS). Download the one for your platform, rename it to `lsp-det` (`lsp-det.exe` on Windows), make it executable, and put it on `PATH`. The binaries are not signed: on macOS, a browser download is quarantined and Gatekeeper refuses to run it until `xattr -d com.apple.quarantine lsp-det` (a `curl` download is not quarantined). Running `lsp-det` with no arguments prints the usage and exits with 2, which is enough to confirm the binary runs
...
| [docs/research/](docs/research/)                       | 25 investigations and measurements: how each language server signals readiness, prior proxies, and the LSP integrations of Serena, Claude Code, Zed, and VS Code |
