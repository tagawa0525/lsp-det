...
- 日付: 2026-08-30
- 状態: 一部廃止（決定 A・B、追補 C、追補 D・E はいずれも 2026-08-30 承認）。決定 A-5、決定 B-4、追補 C-1 と C-5 の `dead`、追補 D は [ADR 0009](0009-success-criterion-and-two-sided-reference.md) が置き換え。`unknown` の追加（A）、`health` による待機終了（B-1〜B-3）、追補 E は生きている
...
### D. 追補（実装の code-review により、上流自身が準拠する場合を定める）

1. **上流の `InitializeResult` に `serverStateProvider` があれば、中継層は拡張 S について透過する**。宣言を足さず、`experimental/serverState` を上流へ転送し、自前の通知も `dead` も出さない。同一接続に送信者の異なる 2 系統（上流の通知と中継層の応答）が流れる状態を避ける。vision.md 4.1 の「上流が拡張 S に準拠していれば透過」と一致する
2. アダプタの有無によらず適用する。アダプタは上流の語彙を補うためのもので、上流が拡張 S を話すなら不要である。中継層の宣言で上流の宣言（例: `{freshness: true}`）を隠すことは仕様 5.1 の趣旨に反する
3. `dead` を出す価値は失われるが、2 系統の混在を避ける方を優先する。内部の状態追跡は続け、ゲートはそれを使う
4. 上流が `initialize` に答えず消えた場合は、準拠しているかを判断できないので、従来どおり宙に浮いたリクエストをエラーで閉じる

### E. 追補（アダプタありでも最初の信号まで `health` は `unknown`）

1. rust-analyzer の最初の `serverStatus` が届く前に `health: ok` を返していた。これは追補 C-1 がアダプタなしで退けたのと同じ観測なしの主張である。**アダプタありも `health` は `unknown` から始め、最初の信号で `ok | warning | error` に置き換える**
2. `readiness` は `initializing` から始める。「initialize 直後」という既知の局面に対応する値が仕様にあるためで、`health` にはその対応物がない
3. 仕様 3 章の `health: unknown` の定義を「観測する手段がない、**またはまだ観測していない**」に広げる。`readiness: unknown` は変えない（readiness には `initializing` がある）

### 却下した案

| 候補                                                                            | 不採用の理由                                                                                                                                                                                                                                                                                                                                                           | 再検討の条件                                                                                                                 |
| ------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------- |
| `readiness` に `failed` を足す                                                  | `health` の `error` か `warning` と同じ事実の二重表現になる。`{health: ok, readiness: failed}` が文法上書けてしまうが、インデックスできないのに結果を信頼できる状態はありえず、意味を持たない。`failed` が持つ情報で `health` に写らないものはない。回復可能性（`Cargo.toml` を直せば再ロードして `ok` に戻る）も `error` と `failed` で変わらず、語を分ける利益がない | サーバーが自己申告する失敗のうち `health` に写らない情報（回復不能と回復可能の機械可読な区別等）を必要とする実例が現れたとき |
