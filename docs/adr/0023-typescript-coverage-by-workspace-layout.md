...
## 追補（2026-09-26）: workspace のフォルダが複数なら宣言しない

規則 R1 は「フォルダが複数なら全部が満たすときだけ宣言する」としていたが、これでは足りない。tsserver は workspace のフォルダを知らず、読み込み済みの project と、そこから solution で辿れる project しか探さない。フォルダが 2 つなら互いに無関係な project が 2 つあるだけで、一方のファイルしか開いていなければ、他方からの参照は答えに現れない。tsls 5.3.0 に直接当てて、フォルダ A と B にそれぞれ全ソースを含む tsconfig を 1 つずつ置き、B が A の関数を import して呼ぶ配置で測った。A のファイルだけを開いた状態では、A の関数の `references` は空だった。B のファイルを開くと、B の呼び出しが現れた。solution のない multi-project の workspace と同じ構造である。

フォルダ同士が参照し合わないなら各フォルダの中の答えは完全だが、それを確かめるには import の解析が要り、tsserver の仕事を観測者が再実装することになる。

2026-09-26 にユーザーが決め、規則 R1 を「workspace のフォルダがちょうど 1 つで、そのフォルダが完全なときだけ宣言する」に改めた。フォルダが複数なら `freshness` だけを宣言する。途中のフォルダの追加で `readiness` を `unknown` にする扱い（決定 4）とも一貫する。一方のフォルダが他方を含む配置は、外側を歩けば設定ファイルが 2 つ見つかるので、改める前から不完全と判定されていた。

却下した案: フォルダ間に import がないと確かめられたときだけ複数でも宣言する（import の解析は tsserver の再実装になる）。今のまま仕様 10 章に欠けとして書く（守れない保証の宣言は仕様 5 章の違反）。
