...
- **compilation database のない workspace**（ADR 0020 追補 2026-09-09。ユーザーの決定）: `initializing` のまま（決定 (a)）は横断リクエストを永久に保留してしまうため改め、観測者が clangd 自身の探索（`GlobalCompilationDatabase.cpp` の `DirectoryBasedGlobalCompilationDatabase`）をなぞってデータベースの所在を読み、なければ `unknown` にする。最初の `textDocument/didOpen` で、開かれたファイルのディレクトリからファイルシステムの根まで上りながら `compile_commands.json`、`build/compile_commands.json`、`compile_flags.txt` の順に探す（上流が `--compile-commands-dir=<dir>` で起動されていれば `<dir>/compile_commands.json` だけを見る）。見つかれば `readiness` は `initializing` のままで、`didOpen` と begin の間の実測 2 ms の隙間は従来どおり観測者の保留が埋める。見つからなければ `unknown`（8.2 の 3）にし、何も保留しない。判定は最初の `didOpen` で一度だけで、2 回目以降の `didOpen` は読まない。begin はいつでも `indexing` にし、end で `ready`（`unknown` にした後の begin でも同じ）。`.clangd`（`CompileFlags.CompilationDatabase`）だけで指定されたデータベースは見ないので、その workspace は `unknown` から始まる（却下した案 (c) の残り）
