...
| 項目                       | 2.1.266 での観測                                                                                                                                                                                                                                                                                                                                                                                                                                           | 前回                   |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| 最初の横断リクエストの時点 | `initialize` の応答の直後に `initialized` → `didOpen` → `references` を 1 ms 以内に連続で送る。CC の `--debug-file` の時刻では `initialize` の応答まで 19 ms（CC 側の計時。プロセスの起動を含む）、記録ラッパーの時刻（ラッパーがサーバーを起動してからの経過）では `initialize` が 0.0 ms、`initialized` と `didOpen` が 5.2 ms、`references` が 5.4 ms。rust-analyzer は 2 ms で `[]` を返し、CC のツール結果は "No references found …"（実際は 2 箇所） | 第 2 回: 6 ms 後       |
