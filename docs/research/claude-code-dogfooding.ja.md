...
## 第 6 回（2026-09-06）: 実害の一事例。空応答を信じたエージェントが使われている関数を消す

第 5 回は「lsp-det を挟むと結果が完全になる」の対照だった。外部レビュー（[external-review-2026-09.md](external-review-2026-09.md) §4）の指摘どおり、上流を動かすには「保留しなければどう間違えたか」の記録が要る。ADR 0018 決定 A-2。

### 方法

第 5 回と同じ道具（入れ子の非対話 CC 2.1.261、Sonnet、tee ラッパー、`git init` だけの被験体、直接 / lsp-det 経由）。lsp-det は main `ad39c33`（保留の開始と解放のログ入り）の release ビルド。指示は 2 種類。

- **指示した版**: 手順を固定する。「`findReferences` を呼び、宣言以外の件数が 0 なら Edit でその関数を消せ。0 でなければ何もするな」。ファイルは読ませない
- **自然な版**: 「使われていない関数を整理して。判断は `findReferences` だけで、ファイルを読んだり grep したりしないこと」

被験体は 2 つ。

- **tsls の起動直後**: `a.ts` に `target`（`b.ts` から使われる）と `other`（未使用）。`target` の参照を問う
- **gopls と Bash で作ったファイル**: `a.go` に `Helper`（最初は未使用）。先に `findReferences` でサーバーを起動させ、Bash で `Helper()` を使う `c.go` を作り、もう一度 `findReferences` を問う。CC は最初の LSP ツール呼び出しでサーバーを起動するので、先に起動させないと `c.go` が起動時から見えてしまう（最初の 4 走行はこの設計ミスで無効。捨てた）

削除の後に `tsc -p .` / `go build ./...` で壊れたかを確かめた。

### 結果

| 走行              | 直接                                                                                                                                                                                                               | lsp-det 経由                                                                                                                                                               |
| ----------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| tsls、指示した版  | `didOpen` の 1 ms 後に `references`。213 ms で宣言だけの応答（usages 0）。**`target` を削除**。`tsc`: `b.ts(1,10): error TS2305: Module '"./a"' has no exported member 'target'.`                                  | 0.244 秒保留（`holding … while {ok, indexing}` → `released … after 0.244s: ready`）。usages 2。削除せず。`tsc` は通る                                                      |
| tsls、自然な版    | 未使用の `other` だけを削除（正しい）。`target` は残った。エージェントが考えてから最初の `references` を出すまでに 4.3 秒かかり、その間にプロジェクトのロードが終わっていた。問い合わせ位置は行頭（`export` の上） | 同じ結果                                                                                                                                                                   |
| gopls、指示した版 | `c.go` を作った後の `references` も 0 件。**`Helper` を削除**。`go build`: `./c.go:3:9: undefined: Helper`                                                                                                         | 2 回目の `references` の前に代行の `didChangeWatchedFiles`（`Created c.go`）が届き 1 件。削除せず。`go build` は通る                                                       |
| gopls、自然な版   | 0 件を 2 回見た後、エージェントが LSP ツールで `c.go` に触れ（CC が `didOpen c.go` を送る）、gopls が `c.go` を見て 1 件。削除せず。回避は偶然                                                                     | 代行の `Created` で 2 件（宣言含む）。削除せず。エージェントは「自分で作ったファイルの参照は自己成就」「`main` は参照 0 でも消せない」と説明して JSON の指示に従わなかった |

費用: 12 走行（無効の 4 つを含む）で 1.5 ドル。

### 結論

- **実害の一事例が取れた**。指示した版では、直接のときに tsls（起動直後の宣言だけの応答）と gopls（知らされていない新規ファイル）の両方でエージェントが使われている関数を消し、ビルドが壊れた。lsp-det 経由では、保留（tsls）と代行（gopls）で正しい件数になり消さなかった
- **害が出るかは、指示ではなく問い合わせまでの時間と手順で決まる**。自然な版は、エージェントが考える数秒の間にロードが終わったり、別のファイルに触れて偶然サーバーに知らせたりして害を免れた。これは「エージェントが遅ければ大丈夫」ではなく「速いエージェントほど嘘を掴む」ということで、指示した版が示す 1 ms 後の問い合わせは CC の実際の挙動（第 2 回: `initialize` 応答の 6 ms 後）である
...
| 項目                       | 2.1.266 での観測                                                                                                                                                                                                                                                                                                                                                                                                                                            | 前回                   |
| -------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| 最初の横断リクエストの時点 | `initialize` の応答の直後に `initialized` → `didOpen` → `references` を 1 ms 以内に連続で送る。CC の `--debug-file` の時刻では `initialize` の応答まで 19 ms（CC 側の計時。プロセスの起動を含む）、記録ラッパーの時刻（サーバーが stdin を読み始めてからの経過）では `initialize` が 0.0 ms、`initialized` と `didOpen` が 5.2 ms、`references` が 5.4 ms。rust-analyzer は 2 ms で `[]` を返し、CC のツール結果は "No references found …"（実際は 2 箇所） | 第 2 回: 6 ms 後       |
...
- 第 6 回の自然な版の結果（害を免れた）はモデルと走行ごとに変わる。指示した版だけが再現可能な記録で、自然な版は「偶然に消えることもある」という但し書き
