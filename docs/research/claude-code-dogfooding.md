...
| ------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------- |
| When the first cross-file request goes out | Right after the `initialize` response: `initialized` → `didOpen` → `references` within 1 ms of each other. On CC's `--debug-file` clock (CC's own timing, including process startup) the `initialize` response arrives at 19 ms; on the wrapper's clock (seconds since the wrapper started the server) `initialize` is at 0.0 ms, `initialized` and `didOpen` at 5.2 ms, `references` at 5.4 ms. rust-analyzer answers `[]` in 2 ms and CC's tool result says "No references found …" (there are 2) | Round 2: 6 ms after    |
