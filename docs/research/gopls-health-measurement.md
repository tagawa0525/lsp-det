...
| ------------------------------------------------------------------------------------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------ |
| b.go の `Target()` の呼び出しをディスク上で消し、`didChangeWatchedFiles` の直後から 10 ms ごとに `references` | 最初の要求（0 ms）から 0 件（正しい） | 送信 0〜40 ms の要求は **1 件（消した呼び出し）**、50 ms の要求から 0 件             |
...
2 走行とも同じ（時刻は要求の送信時刻。要求は応答を待たずに 10 ms ごとの予定時刻で送り、答えは id で対応づける。burst の 20 通知は最初の 1 つを要求より先に送り、残りは専用スレッドで 30 ms ごとに送る。予定からのずれは最大 0.6 ms）。今の gopls は通知を受けた順に処理するので、`didChangeWatchedFiles` の後の要求は必ずその変更を織り込む。CL 830844 はこの順序を要求に対しては守らない（編集通知 `didOpen` / `didChange` / `didSave` / `didClose` が来たときだけ保留分を同期に flush する）。変更を受け取っておきながら最長 50 ms、連続する変更では 500 ms、古い状態で答え、その間クライアントに信号はない（flush の時点で出るものはなく、`publishDiagnostics` は再ロードの後）。仕様 6 章 2 項の freshness（受け取った `didChangeWatchedFiles` を以後の要求が織り込む）が成り立たない。
