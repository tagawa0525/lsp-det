...
- **readiness**: `initializing` から、token が `nil/loadNixosOptionsProgress`、`nil/loadInputFlakeProgress`、`nil/flakeArchiveProgress` のいずれかの begin で `indexing`、未完了の token が 0 になった end で `ready`。他の token は読まない。begin が来ない workspace（flake がない、`flake.lock` がない、`nixpkgs` の入力がない、入力の store path がない）の扱いは **(b)**（2026-09-09、ADR 0021 追補。ユーザーの決定）: `initialize` の `workspaceFolders` を読み（`learn_workspace_folders`）、いずれかのフォルダの `flake.lock` の root ノードの入力に `nixpkgs`（既定の `nix.nixpkgsInputName`）があるときだけ `initializing` から始め、それ以外は `unknown` から始める。begin を一度も見ていない状態で届いた type 2 の `window/showMessage`（store path のない入力を示す。実測では読み込みは走らず begin は来ない）も readiness を `unknown` にする。どちらの `unknown` からも、その後に begin が来れば通常どおり `indexing` → `ready` になる。却下したのは (a) clangd と同じく `initializing` のまま（flake のない Nix のディレクトリでは文書内で完結する `references` も永遠に返らず、使いにくい）と、最初の begin まで一律 `unknown`（flake.lock のある通常の workspace でも読み込み前の約 100 ms の窓で宣言しないクライアントの保留が失われる）
...
- **クライアントの宣言との関係**: この保留は `flake.lock` に `nixpkgs` の入力があり readiness が `initializing` から始まる workspace に限る。flake.lock の読み込みの前の窓（約 100 ms。信号なし）に届いた入力への `definition` は空で返る。`experimental/serverState` を宣言しないクライアントには lsp-det が `initializing` の間その要求を保留し（9 章の代行）、最初の begin と end の後に転送するので完全な答えになる（実サーバーテストで確認）。宣言するクライアントには 9 章のとおり転送し、待つのはクライアントの責務。Claude Code は宣言しないので前者。readiness が `unknown` から始まる workspace（決定 (b)）では保留は効かず、7.0 の要求はそのまま転送される（8 章の表）
