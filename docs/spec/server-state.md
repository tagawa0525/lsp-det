...
| pyright       | `window/logMessage` のファイル列挙完了（"Found N source files" / "No source files found."）。`$/progress` は開いたファイルの解析の進行で、横断リクエストの完全性とは別 | 列挙完了 → `readiness: "ready"`（観測者による合成）。health の信号はなく `unknown`               | completeness（見込み。準拠テストは M5 で当てる）                                             |
| tsserver 系   | `$/progress`（クラッシュ時も end）                                                                                                                                     | 観測者がログ・接続監視と併用して合成                                                             | completeness のみ（非同期処理のため freshness 不可）                                         |
