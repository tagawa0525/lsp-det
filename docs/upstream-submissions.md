...
2. Claude Code: anthropics/claude-code#76870 に「待つべき信号は `experimental/serverState`。lsp-det が今日の橋」を提案し、第 1〜7 回の実測を添える。anthropics/claude-code#82416 に tsls の PR へのリンクと再現。anthropics/claude-code#85225 に第 5 回の観測。**提出済み（2026-09-09）**: コメント 4 件と新規 issue 2 件（anthropics/claude-code#93103、#93104）。`workspace/configuration` は anthropics/claude-code#16360 へのコメント。新規は `shutdown` の `params: {}` と、`didClose`（anthropics/claude-code#64276 はロック済みでコメントできない）の 2 件
