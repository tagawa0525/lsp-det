...
| rust-analyzer              | issue で `experimental/serverStatus` への field 追加（`readiness`）と別通知の `experimental/serverState`（fork `tagawa0525/rust-analyzer` の `server-state`）の両案を並べ、相手が選んだ方を PR にする。`quiescent` の飛び越えの実測を根拠にする                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | 仕様 10 章、[research/rust-analyzer-quiescent-measurement.md](research/rust-analyzer-quiescent-measurement.md)                                                                                                                                                                                                                     | 第 2 段。準備 4。rust-lang/rust-analyzer#10888 は「`serverStatus` で既にある」で閉じている                      | **提出済み**（rust-lang/rust-analyzer#23331、2026-09-10。両案。fork の両ブランチは upstream master f312032107 に rebase 済み: `server-status-readiness` 3807709227、`server-state` 45a86fa0ed。出した文面は下の草案の節にそのまま残す）。相手が選んだ方の PR を出す                                                                |
...
Today, the request sent 1ms after the notification already answers 0 references: gopls handles the notification before the request that follows it. With the CL, the same request answers 1 reference, the call that no longer exists, and keeps doing so for 52ms, until the debounce flushes. Sending the notification 20 times 30ms apart (a burst, the case the CL is for) after putting the call back, the answer stays at 0 for 507–511ms, the `debounceMax`, while the file on disk has had the call the whole time. Two runs each, same numbers.
