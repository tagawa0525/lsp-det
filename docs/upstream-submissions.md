...
次: fork の `server-status-ready` を push し、PR を出す（ユーザー確認のうえで）。issue には PR のリンクを 1 行で返す。PR の草案:
...
`cargo xtask tidy` and the `rust-analyzer` lib tests pass.
````
