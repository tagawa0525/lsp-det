...
2. Claude Code: anthropics/claude-code#76870 に「待つべき信号は `experimental/serverState`。lsp-det が今日の橋」を提案し、第 1〜7 回の実測を添える。anthropics/claude-code#82416 に tsls の PR へのリンクと再現。anthropics/claude-code#85225 に第 5 回の観測。`workspace/configuration` は anthropics/claude-code#16360 へのコメント。新規は `shutdown` の `params: {}` と、`didClose`（anthropics/claude-code#64276 はロック済みでコメントできない）の 2 件
3. pyright: `serverInfo` の enhancement request（issue）。PR は返事の後（fork の `server-info` を上流 HEAD に rebase してから）。草案は下
...
````markdown
The second half of this (empty results after tsserver dies, no recovery) has a fix upstream: typescript-language-server/typescript-language-server#1125. When tsserver is killed by a signal (SIGKILL, or Node's own OOM abort, `{code: null, signal: 'SIGABRT'}`), `exitCode` is `null`, the `if (exitCode)` in `onExit` skips the shutdown, and the language server stays alive answering `[]` to every `references`. The PR makes it exit as it already does for a non-zero exit code, so the client sees the process die and can respawn. Reproduced on 5.3.0 and 6.0.0: `references` returns 2 locations; `kill -KILL <tsserver pid>`; the next `references` returns `[]` with no error, and the only trace is a type 1 `window/logMessage` `[tsserver] Exited. Code: null. Signal: SIGKILL`.

The first symptom (a server that never loaded a project and answers empty) is not covered by that PR. Two things a client can do today: treat a type 1 `window/logMessage` matching `[tsserver] Exited` as a dead server, and treat an empty `references` before the server has reported readiness as "unknown" rather than "none". There is no standard signal for the latter; I am proposing one (`experimental/serverState`, see my comment on #76870), and lsp-det (https://github.com/tagawa0525/lsp-det) implements it as a proxy in front of typescript-language-server. After the crash above, the same `references` through it comes back as an error (`health: error`, with the tsserver exit message) instead of `[]`.
````
...
The LSP client sends the `shutdown` request with `"params": {}`. LSP defines `shutdown` with no params. rust-analyzer (the `lsp-server` crate deserializes the params as unit) answers with an error, `-32602 Failed to deserialize shutdown: invalid type: map, expected unit; {}`, Claude Code logs `Failed to stop LSP server '…': …` and disconnects without sending `exit`. The server is left to notice the closed pipe on its own. gopls, pyright and nixd accept the extra `{}`, so this is visible only with strict servers, but the official rust-analyzer plugin hits it on every session end.
