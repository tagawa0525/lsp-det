...
> 英語版（[vision.md](vision.md)）が正で、本文書はその日本語版（ADR 0017 追補 F）。

## 本文書の位置づけ

本文書は 3 拡張（宣言範囲・サーバー状態・起動マニフェスト）の仕様化と標準化に関する**長期構想**である。サーバー状態の規範は [spec/server-state.md](spec/server-state.md)（日本語版は [spec/server-state.ja.md](spec/server-state.ja.md)）、現在の実装スコープは [v0.1-design.ja.md](v0.1-design.ja.md) に定義する。本文書のうち、5 章（上流への提案経路）と 6.7 の標準化用調査タスクは凍結中であり、v0.1 が安定稼働した後に再開を判断する。スコープ選定の理由は [adr/0001-tool-first-readiness-gate.md](adr/0001-tool-first-readiness-gate.md) と [adr/0003-extension-s-zero-based.md](adr/0003-extension-s-zero-based.md) を参照。
...
規範は [spec/server-state.md](spec/server-state.md) を正とする（日本語版は [spec/server-state.ja.md](spec/server-state.ja.md)）。要点:
...
- 相方サーバーとの合成（Vue の Hybrid Mode のように、横断の答えを別プロセスの TS サーバーが出す構成）はクライアントの責務。クライアントは接続ごとの `ServerState` を AND すれば足り、観測者（lsp-det）は隣の接続を知らなくてよい。Vue で実測し、横断の答えを出す接続の保留だけで結果が完全になった（[research/vue-composition-measurement.md](research/vue-composition-measurement.md)、[adr/0019](adr/0019-v0.4-corpus-and-counterexamples.md) 決定 B-5）
...
3. LSP 本体（microsoft/language-server-protocol）への proposal は、上流の取り込みを待たず、語彙の乱立の実測（[research/readiness-vocabulary-corpus.ja.md](research/readiness-vocabulary-corpus.ja.md)）を根拠に出す。既存 issue microsoft/language-server-protocol#511 のスレッドに「エージェント用途からの再提案」として接続する。宣言範囲と起動の宣言は凍結中で、この経路の対象外
