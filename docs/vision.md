...
## 本文書の位置づけ（2026-08-28 改訂）

本文書は当初「仕様草案 v0.1」として書かれた。実装に着手する前の設計検討で、**仕様先行ではなくツール先行**で進めることを決めたため、本文書は背景・コンセプト・3 拡張の構想・先行調査・標準化経路を記録する**長期構想**として保存し、直近の実装スコープは [v0.1-design.md](v0.1-design.md) に定義する。

確定した進め方:

- 進め方: プロキシを「今日使える道具」として磨き、自分の環境（Claude Code + Serena）で実証してから標準化を再評価する
- v0.1 の中核は**拡張 B（準備完了）**。solidlsp の実測でも起動・準備完了系の補正が範囲補正より圧倒的に多く、先行実装（rust-analyzer `serverStatus`、jdtls `language/status`）があり、上流の移行コストが最小
- 拡張 A（宣言範囲）は v0.1 から除外（パーサ同梱の判断ごと延期）。拡張 C（起動マニフェスト）は argv 指定に縮退し、レジストリ構想は凍結
- 5 章の上流提案経路と 6.7 の標準化用調査タスクは凍結中。v0.1 が 1 ヶ月安定稼働したら再開を判断する
