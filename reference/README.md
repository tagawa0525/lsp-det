...
| ruff                       | https://github.com/astral-sh/ruff                                        | e7adf82  | `ruff server` が扱うリクエストの一覧 (crates/ruff_server/src/server/api.rs, api/requests/)。diagnostics・formatting・code actions・hover のみで definition/references/workspace symbol はない (ADR 0021 決定 A-4)                                           |
| nixd                       | https://github.com/nix-community/nixd                                    | 6f05dff  | `$/progress` が 2 本並行する評価 worker（nixd/lib/Controller/LifeTime.cpp）、同期ループの worker が要求を待たせる構造（lspserver/src/Connection.cpp、lib/Eval/AttrSetProvider.cpp）、単一文書の `references`（lib/Controller/FindReferences.cpp）。ADR 0021 |
| nil                        | https://github.com/oxalica/nil                                           | 205c8ba  | flake.lock の読み込みと NixOS のオプション・入力 flake の評価の `$/progress`（crates/nil/src/server.rs）、snapshot に答える要求の経路、単一文書の `references`（crates/ide/src/ide/references.rs）。ADR 0021                                                |
...
| ディレクトリ               | URL                                                     | 取得時点 | 参照目的                                                                                                                                                                                                    |
| -------------------------- | ------------------------------------------------------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| serena                     | https://github.com/oraios/serena                        | 701e7c8  | solidlsp の言語別補正の実測記録。`ls_specific_settings` / `ls_base_cmd` (dependency_provider.py)、gopls ハードコード (language_servers/gopls.py)、待ち時間フック。2026-09-09 に上流 HEAD へ進めた（準備 6） |
