...
| rust-analyzer                            | （別案）`experimental/serverStatus` に `ready: bool` の field を足す（rust-lang/rust-analyzer#23331 でメンテナが選んだ形）。写像は field があればそれを読む  | `rust_analyzer_reports_ready_in_server_status`                                                         |
...
| rust-analyzer              | `tagawa0525/rust-analyzer` の `server-status-ready`（rust-lang/rust-analyzer#23331 でメンテナが選んだ、簡略化した案 A。rust-lang/rust-analyzer#23362 として提出。issue が指す 3 値版の `server-status-readiness` は残す） | `ServerStatusParams` に `ready: bool`（`reload.rs` の `is_ready`。ワークスペースの読み込み中は `false`、読み込み後は priming 中も `true`）、初期の `last_reported_status` は `ready: false`（通知の回数は変わらない）、`lsp-extensions.md` の field と「この field は人向けでなく答えを信じてよいかを決めるクライアント向け」の注記と hash、`editors/code` の型、遷移を観る slow-tests 2 件              | 通過（`cargo xtask tidy`、lib tests、slow-tests 2 件。CI 専用ブランチで `github.repository` の条件を外した fork の CI の全ジョブも。tagawa0525/rust-analyzer#2）。写像が field を読み、7.1 / 7.2 / 7.3 も通る。実測は `scripts/rust-analyzer/status-probe.py` |
