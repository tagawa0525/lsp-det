...
| typescript-language-server | `tagawa0525/typescript-language-server` の `no-server-request-failed`（上流 master 起点。`server-info` とは独立） | `TsClient.execute` / `executeCustom` が、tsserver が動いていないとき `NoServer` の空応答ではなく `RequestFailed`（-32803、message に終了理由）で reject する。`ts-client.test.ts` に 2 件                                          | 通過（`typescript_language_server_fails_requests_after_tsserver_exit`。素の 5.3.0 では `result: []` で失敗する）。vitest 141 件と lint も通る |
