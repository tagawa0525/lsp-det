...
// デッドロックの危険があり避けている。この効果はユニットテストの対象外とし、
// 手動 smoke テストで検証済み (2026-08-28): (1) プロキシを SIGKILL すると
// pre_exec で pdeathsig を設定した上流プロセスも道連れで終了する、
// (2) プロキシの親 (擬似クライアント) を SIGKILL すると
// `set_self_pdeathsig()` によりプロキシ自身も終了する。両者を確認済みなので、
// 「クライアント死亡 → プロキシ死亡 → 上流死亡」の連鎖も論理的に成立する。
